...
export async function getSelectedMenuData(menuId: number) {
    const result = await pool.query(
        `
        SELECT 
            menu_id, 
...
export async function getProductDetailData(productId: number) {
    ...
    p.image_alt,
    (
        SELECT ARRAY_AGG(DISTINCT i.image_filename)
        FROM images AS i
        WHERE i.product_id = p.item_code
    ) AS image_filenames,
...
export async function getGridProductData() {
    const result = await pool.query(
        `
            SELECT 
                p.product_id, 
