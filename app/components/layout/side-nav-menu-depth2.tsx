...
export default memo(function SideNavMenuDepth2({
  expandedDepth2,
  expandedDepth3,
  setExpandedDepth2,
  setExpandedDepth3,
      ...
      <ul>
        ...
        {depth2Menus.map((depth2, i) => {
          const depth3Menus = menus.filter(
            (menu) =>
              Number(menu.depth_level) === 3 &&
              menu.parent_menu_id === depth2.menu_id,
          );
          return (
            ((expandedDepth2 === index && expandedDepth3 === null) ||
              expandedDepth3 === i) && (
              <li
                key={depth2.menu_id}
                className={clsx(styles["menu__item"], {
                  [styles["menu__item--border"]]: depth2.border,
                })}
              >
                ...
                {depth3Menus.length === 0 && (
                  <Link
                    href={depth2.path ? depth2.path : depth2.link || "#"}
                    className={clsx(styles["menu__depth2"])}
                  >
                    {depth2.depth2}
                  </Link>
